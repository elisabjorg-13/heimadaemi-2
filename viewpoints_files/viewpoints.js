...
window.onload = function init()
{
    ...
    canvas.addEventListener("mousemove", function(e){
        if(movement) {
            userAngle += 0.7*(origX - e.clientX);
            userAngle %= 360.0;
            userXDir = 360*Math.sin( radians(userAngle));
            userZDir = 360*Math.cos( radians(userAngle));
            origX = e.clientX;
        }
    } );
    ...
    window.addEventListener("keydown", function(e){
        switch( e.keyCode ) {
            ...
            case 57:	// 8: from beside the car
                view = 9;
                document.getElementById("Viewpoint").innerHTML = "9: í föstum punkti að horfa á bílinn";
                break;
            case 48:	// 8: from beside the car
                view = 0;
                document.getElementById("Viewpoint").innerHTML = "0: fost i z og allskonar";
                break;
            ...
            case 40:
            height -= 2.0;
            document.getElementById("Height").innerHTML = "Viðbótarhæð: "+ height;// down arrow
            userXPos -= userIncr * userXDir;
            userZPos -= userIncr * userZDir;;
            ydist -=1;
                break;
...
function render()
{
  ...
  case 9:
      // View from x looking at car
      mv = lookAt( vec3(-40.0, 140.0, 50+height), vec3(carXPos, carYPos, 0.0), vec3(0.0, 0.0, 1.0 ) );
      drawScenery( mv );
      mv = mult( mv, translate(carXPos, carYPos, 0.0) );
      mv = mult( mv, rotateZ( carDirection ) ) ;
      drawCar( mv );
    break;
  case 0:
      mv = lookAt( vec3(100+userXPos, userZPos, 5), vec3(userXDir+userXPos, userZDir+userZPos, 0.5 ), vec3(0.0, 0.0, 1.0 ) );
      drawScenery( mv );
      mv = mult( mv, translate(carXPos, carYPos, 0.0) );
      mv = mult( mv, rotateZ( carDirection ) ) ;
      drawCar( mv );
