...
function render()
{
    ...
    rotDay += 0.5;
    rotYear += 0.2;        // not the correct value, but looks better!
    ...
    mv1 = mult( mv, translate( 0.0, -0.4, 0.0 ) );
     mv1 = mult( mv1, scalem( 0.7, 0.05, 0.5 ) );
    gl.uniformMatrix4fv(matrixLoc, false, flatten(mv1));
    gl.drawArrays( gl.TRIANGLES, 0, numVertices );


    mv1 = mult( mv, translate( 0.0, -0.2, 0.0 ) );
     mv1 = mult( mv1, scalem( 0.1, 0.4, 0.1 ) );
    gl.uniformMatrix4fv(matrixLoc, false, flatten(mv1));
    gl.drawArrays( gl.TRIANGLES, 0, numVertices );

    mv1 = mult(mv, rotateY(rotYear));
    mv1 = mult(mv1, rotateZ(16.0*Math.sin(rotDay*0.1)));
    mv1 = mult( mv1, translate( 0.0, 0.0, 0.0 ) );
    mv1 = mult( mv1, scalem( 1.5, 0.1, 0.1 ) );
    gl.uniformMatrix4fv(matrixLoc, false, flatten(mv1));
    gl.drawArrays( gl.TRIANGLES, 0, numVertices );
